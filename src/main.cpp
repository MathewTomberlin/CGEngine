...
int main() {
    ...
    ScriptEvent tileClickEvt = [&clickedTilemapPos, &assignedType](ScArgs args) {
    ...
    ScriptEvent loadTilemapDataEvt = [&tilemapDataPath](ScArgs args) {
    ...
    ScriptEvent toolboxTileClickEvt = [&assignedType](ScArgs args) {
        MouseReleaseInput* mouseEvt = args.script->pullOutInputPtr<MouseReleaseInput>();
        Tilemap* tilemap = args.caller->get<Tilemap*>();
        if (mouseEvt == nullptr || tilemap == nullptr) return;
    ...
    ScriptEvent tilemapConstruction = [&tilemapDataPath, &saveTilemapDataEvt, &tileClickEvt, &loadTilemapDataEvt, &tileTypes, &toolboxTileClickEvt, &clickedTilemapPos, &gridId](ScArgs args) {
        ...
        ScriptEvent clickUpdateScript = [&clickedTilemapPos, &gridId](ScArgs args) {
            MouseReleaseInput* mouseEvt = args.script->pullOutInputPtr<MouseReleaseInput>();
    ...
    ScriptEvent playerConstruction = [&lastMove, &facing](ScArgs args){
